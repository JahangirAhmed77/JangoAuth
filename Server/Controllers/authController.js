...
import brevoConfig from "../config/nodeMailer.js"
import { EMAIL_VERIFY_TEMPLATE,PASSWORD_RESET_TEMPLATE } from '../config/emailTemplates.js'
import axios from 'axios'; // Import axios

const backendurl = process.env.BACKEND_URL; // Make sure BACKEND_URL is set in your environment variables
...
export const register = async (req, res) => {
    ...
    try {
        ...
        // Sending Welcome Email (Brevo)
        const mailOptions = {
            from: process.env.SENDER_EMAIL,
            to: email,
            subject: 'Welcome to JangoAuth',
            text: `Welcome to JangoAuth. Your account has been created with email: ${email}`,
        };

        await brevoConfig.sendBrevoEmail(mailOptions);
...
export const logout = async (req, res) => {
    try {
        ...
        console.log("Clear cookie called"); // add this line
    } catch (error) {
        console.error("Logout error:", error); // Add this line
        res.json({ success: false, message: error.message });
    }
};
...
export const verifyEmail = async (req, res) => {
    ...
    try {
        const user = await userModel.findById(req.user.id); // Use JWT user id
        if (!user) {
            return res.status(404).json({ success: false, message: "user not found" });
        }
...
export const verifyAccount = async (req, res) => {
    ...
    try {
        ...
        // Send OTP and user ID to the backend verification endpoint
