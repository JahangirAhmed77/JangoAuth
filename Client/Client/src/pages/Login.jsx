    ...
    try {
      ...
      if (state === 'Sign up') {
        ...
        if (data.success) {
          ...
          setisloggedin(true)
          getUserData()
          await axios.post(`${backendurl}/api/auth/send-verification-otp`)
    ...
    <div className='flex flex-col  justify-center items-center min-h-screen px-6 sm:px-0 bg-[url("/bg_img.png")] bg-cover bg-center'>
      ...
      <div className=' bg-slate-900 p-10 mt-15 rounded-lg shadow-lg w-full sm:w-96 text-white text-sm'>
        ...
        <form onSubmit={submitHandler}>
          ...
          {state !== 'Sign up' && <p onClick={() => navigate('/reset-password')} className='mb-4 text-[#66CCCC] cursor-pointer'>Forgot Password</p>}
